...
import net.tidalhq.tidal.event.impl.MacroStartedEvent;
...
public class UnexpectedLocationFailsafe extends Failsafe {
    ...
    @Override
    public String getName()        { return "Unexpected Location Failsafe"; }

    @Override
    public String getDescription() { return "Stops macro if the player moves to an unexpected location"; }

    @Override
    public String getId()          { return "unexpected_location"; }
    ...
    public void onClientEndTick(ClientEndTickEvent event) {
        ...
        if (!armed) {
            if (current.equals(expectedLocation)) {
                armed = true;
            }
            return;
        }
